...
class ApplControllerTestBank:
    ...
    def init_messages(self):
        ...
        self.messages.append(UserMessage("Il mio dataset contiene i seguenti attributi  age, job, marital, education, default, balance, housing, loan, contact, day, month, duration, campaign, pdays, previous, poutcome, y."
                                         "Devo raggiungere il seguente obiettivo: predire se un cliente sottoscriverà oppure no un deposito bancario. Quale fra gli attributi individui come variabile target?", {}))
        self.messages.append(UserMessage('Carica il dataset bank. Restituisci solo con la stringa json pura: {{"tool_calls": [{{"name": "load_dataset", "args": {{"dataset_name": "dset"}}}}]}}. Sostituisci dset con il nome del mio dataset bank. Restituisci solo la stringa json pura. Non aggiungere ne testo ne formattazione', {}, 'load_dataset'))
        self.messages.append(UserMessage('Pulisci il dataset. Rispondi solo con la stringa json {{"tool_calls": [{{"name": "clean_dataset" , "args": {{"dataset_name": "dset"}}}}]}}. Sostituisci dset con il nome del mio dataset bank.  Restituisci solo la stringa senza formattazione ed altro testo.', {}, 'clean_dataset'))
        #self.messages.append(UserMessage('Disegna i grafici delle distribuzioni di tutti gli attributi del dataset bank. Rispondi solo con la stringa json {{"tool_calls": [{{"name": "clean_dataset" , "args": {{"dataset_name": "dset"}}}}]}}. Sostituisci dset con il nome del mio dataset bank.  Restituisci solo la stringa senza formattazione ed altro testo.', {}))
        self.messages.append(UserMessage('Calcola la distribuzione di tutti gli attributi del dataset bank. Restituisci esattamente la seguente stringa json con i parametri sostituiti: {{"tool_calls": [{{"name": "calculate_the_distributions_of_all_attributes", "args": {{"dataset_name": "dset"}}}}]}}. Sostituisci dset con il nome del mio dataset. Restituisci solo la stringa json pura.',{}, 'calculate_the_distributions_of_all_attributes'))
        self.messages.append(UserMessage('Esegui la codifica del mio dataset bank. Restituisci esattamente la seguente stringa json con i parametri sostituiti: {{"tool_calls": [{{"name": "esegui_codifica_dataset", "args": {{"dataset_name": "dset"}}}}]}}. Sostituisci dset con il nome del mio dataset. Restituisci solo la stringa json pura.', {}, 'esegui_codifica_dataset'))
        ...
        self.messages.append(UserMessage("Il mio obiettivo è predire se un cliente sottoscriverà oppure no un deposito bancario. Di quale problema di learning si tratta (classificazione, regressione, clustering.....)? Rispondi nella forma: 'Si tratta di un problema di learning di [tipo_di_problema]'", {}))
        ...
        self.messages.append(UserMessage("Il mio dataset contiene gli attributi seguenti: age, job, marital, education, default, balance, housing, loan, contact, day, month, duration, campaign, pdays, previous, poutcome, y"
                                   "Quali di questi sono considerabili critici sensibili per il mio obiettivo e potenzialmente fonte di bias, unfairness, disparità?", {}))
